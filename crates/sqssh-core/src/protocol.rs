    ...
    pub fn encode(&self) -> Vec<u8> {
        match self {
            Self::Ok { message } | Self::Error { message } => {
                let typ = if matches!(self, Self::Ok { .. }) {
                    CTL_RESP_OK
                } else {
                    CTL_RESP_ERROR
                };
...
pub async fn decode_auth_response(recv: &mut quinn::RecvStream) -> Result<AuthResponseData> {
    ...
    match type_buf[0] {
        ...
        other => Err(Error::Protocol(format!(
            "unknown auth response type: {other:#x}"
        ))),
    }
}
...
impl RawShellHeader {
    ...
    pub async fn decode(recv: &mut quinn::RecvStream) -> Result<Self> {
        ...
        recv.read_exact(&mut term_len_buf)
            .await
...
impl ShellControlMsg {
    ...
    pub async fn decode(recv: &mut quinn::RecvStream) -> Result<Self> {
        ...
        match type_buf[0] {
            ...
            other => Err(Error::Protocol(format!(
                "unknown shell control type: {other:#x}"
            ))),
        }
    }
}
...
pub async fn decode_manifest_response(recv: &mut quinn::RecvStream) -> Result<Vec<ManifestEntry>> {
    ...
    for _ in 0..count {
        ...
        recv.read_exact(&mut type_buf)
            .await
            .map_err(|e| Error::Connection(format!("manifest entry: {e}")))?;
        let is_dir = type_buf[0] == 1;

        let mut path_len_buf = [0u8; 2];
        recv.read_exact(&mut path_len_buf)
            .await
            .map_err(|e| Error::Connection(format!("manifest path len: {e}")))?;
        let path_len = u16::from_be_bytes(path_len_buf) as usize;

        let mut path_buf = vec![0u8; path_len];
        recv.read_exact(&mut path_buf)
            .await
            .map_err(|e| Error::Connection(format!("manifest path: {e}")))?;
        let path =
            String::from_utf8(path_buf).map_err(|_| Error::Protocol("invalid UTF-8".into()))?;

        let mut meta_buf = [0u8; 28];
        recv.read_exact(&mut meta_buf)
            .await
            .map_err(|e| Error::Connection(format!("manifest meta: {e}")))?;
...
pub async fn decode_transfer_result(recv: &mut quinn::RecvStream) -> Result<(bool, String)> {
    ...
    let message = if msg_len > 0 {
        ...
        recv.read_exact(&mut msg_buf)
            .await
            .map_err(|e| Error::Connection(format!("result msg: {e}")))?;
        ...
        match cmd_type {
            ...
            SFTP_CMD_MKDIR => {
                ...
                let path = String::from_utf8(payload[2..2 + path_len].to_vec())
                    .map_err(|_| Error::Protocol("invalid UTF-8".into()))?;
                let mode =
                    u32::from_be_bytes(payload[2 + path_len..2 + path_len + 4].try_into().unwrap());
    ...
    pub async fn decode(recv: &mut quinn::RecvStream) -> Result<Self> {
        ...
        match type_buf[0] {
            ...
            SFTP_RESP_DIR_LISTING => {
                ...
                for _ in 0..count {
                    let mut flags = [0u8; 1];
                    recv.read_exact(&mut flags)
                        .await
                        .map_err(|e| Error::Connection(format!("dir entry: {e}")))?;
                    let is_dir = flags[0] == 1;
                    let mut pl = [0u8; 2];
                    recv.read_exact(&mut pl)
                        .await
                        .map_err(|e| Error::Connection(format!("dir path len: {e}")))?;
                    let path_len = u16::from_be_bytes(pl) as usize;
                    let mut pb = vec![0u8; path_len];
                    recv.read_exact(&mut pb)
                        .await
                        .map_err(|e| Error::Connection(format!("dir path: {e}")))?;
                    let path = String::from_utf8(pb)
                        .map_err(|_| Error::Protocol("invalid UTF-8".into()))?;
                    let mut meta = [0u8; 28];
                    recv.read_exact(&mut meta)
                        .await
                        .map_err(|e| Error::Connection(format!("dir meta: {e}")))?;
                    entries.push(ManifestEntry {
                        path,
            ...
            SFTP_RESP_STAT_RESULT => {
                ...
                recv.read_exact(&mut pl)
                    .await
                    .map_err(|e| Error::Connection(format!("stat path len: {e}")))?;
                let path_len = u16::from_be_bytes(pl) as usize;
                let mut pb = vec![0u8; path_len];
                recv.read_exact(&mut pb)
                    .await
                    .map_err(|e| Error::Connection(format!("stat path: {e}")))?;
                let path =
                    String::from_utf8(pb).map_err(|_| Error::Protocol("invalid UTF-8".into()))?;
                let mut meta = [0u8; 29];
                recv.read_exact(&mut meta)
                    .await
                    .map_err(|e| Error::Connection(format!("stat meta: {e}")))?;
                Ok(Self::StatResult {
                    path,
                    size: u64::from_be_bytes(meta[0..8].try_into().unwrap()),
                    mode: u32::from_be_bytes(meta[8..12].try_into().unwrap()),
...
impl AgentRequest {
    ...
    pub async fn decode_async(recv: &mut (impl tokio::io::AsyncReadExt + Unpin)) -> Result<Self> {
        ...
        match type_buf[0] {
            AGENT_ADD_KEY => {
                let mut seed = [0u8; 32];
                recv.read_exact(&mut seed)
                    .await
                    .map_err(|e| Error::Connection(format!("agent add seed: {e}")))?;
                let mut clen = [0u8; 2];
                recv.read_exact(&mut clen)
                    .await
                    .map_err(|e| Error::Connection(format!("agent add comment len: {e}")))?;
                let cl = u16::from_be_bytes(clen) as usize;
                let mut cb = vec![0u8; cl];
                if cl > 0 {
                    recv.read_exact(&mut cb)
                        .await
                        .map_err(|e| Error::Connection(format!("agent add comment: {e}")))?;
                }
                Ok(Self::AddKey {
    ...
    pub fn decode(reader: &mut impl std::io::Read) -> Result<Self> {
        ...
        match type_buf[0] {
            ...
            AGENT_RESP_ERROR => {
                ...
                Ok(Self::Error {
                    message: String::from_utf8(mb).unwrap_or_default(),
                })
            }
            AGENT_RESP_KEYS => {
                let mut count_buf = [0u8; 4];
