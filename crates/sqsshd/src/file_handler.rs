...
use sqssh_core::protocol::{self, ManifestEntry, RawFileHeader, RAW_CHUNK_SIZE, RAW_DOWNLOAD_DATA};
...
pub async fn handle_raw_upload(
    ...
    let target =
        validate_path(Path::new(&home), &header.path).map_err(|e| format!("invalid path: {e}"))?;
...
pub async fn receive_raw_file(
    ...
    if written != size {
        tracing::warn!(
            "size mismatch for {}: expected {size}, got {written}",
            path.display()
        );
    }
...
async fn send_file_raw(
    ...
    // Write header
    send.write_all(&header.encode_download())
        .await
...
pub async fn handle_raw_upload_chunk(
    ...
    if header.mtime > 0 {
        let times = [
            libc::timeval {
                tv_sec: header.atime as libc::time_t,
                tv_usec: 0,
            },
            libc::timeval {
                tv_sec: header.mtime as libc::time_t,
                tv_usec: 0,
            },
        ];
        unsafe {
...
pub async fn handle_raw_download(
    ...
    let source = validate_path(Path::new(&home), path).map_err(|e| format!("invalid path: {e}"))?;
    ...
    if meta.is_dir() {
        ...
        let sem = Arc::new(tokio::sync::Semaphore::new(std::cmp::min(
            jobs as usize,
            64,
        )));
        ...
        for i in 0..jobs {
            ...
            let handle = tokio::spawn(async move {
                send_file_chunk_raw(
                    &conn, &file, &fname, file_size, mode, mtime, atime, offset, length,
                )
                .await
            });
            handles.push(handle);
